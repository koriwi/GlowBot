...
- Only the current user message is included automatically. When the request depends on earlier chat context, call `get_recent_messages(count)` to retrieve prior visible user/assistant messages from the database. Do not call it for self-contained requests.
