...
- GSM-7 output is split into standalone messages of at most 160 septets (extension-table characters count as two). Necessary non-GSM output is split at 70 UTF-16 units. Huawei delivery-confirmation inbox entries are marked read and never routed to the LLM, preventing multipart sends from triggering a spurious second reply. MMS is not supported or attempted.
