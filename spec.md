...
- `dms.<chat_id>.forward_sms_to_telegram: true` mirrors that contact's incoming and successfully sent SMS text to the mapped Telegram DM without duplicating it in LLM history. Mirror messages carry a reserved `[GlowBot SMS mirror]` display-only envelope that the Telegram update handler ignores independently of sender metadata, so relays cannot feed them back into the LLM as a second user turn. It defaults to `false` independently for each DM. Modem inbox entries are deduplicated in memory after successful processing, so a delayed/failed mark-read operation cannot trigger the LLM or send a reply twice.
...
- GSM-7 output is split into standalone messages of at most 160 septets (extension-table characters count as two). Necessary non-GSM output is split at 70 UTF-16 units. Segment submissions are paced by two seconds because Huawei modems send asynchronously, and a failed segment is retried up to three times with a three-second delay. Once the LLM turn completes, the incoming message is marked processed even if reply delivery ultimately fails, preventing an unread inbox entry from rerunning and duplicating the entire LLM turn. Huawei delivery-confirmation inbox entries are marked read and never routed to the LLM. MMS is not supported or attempted.
...
- `provider: codex` uses OpenAI's Codex Responses endpoint with OAuth credentials generated by the official `codex login` command, so requests consume the user's ChatGPT/Codex subscription allowance. Pi's `~/.pi/agent/auth.json` is also supported when it contains an `openai-codex` login. GlowBot refreshes expired access tokens and writes updated credentials back to `auth_file` in its original format.
